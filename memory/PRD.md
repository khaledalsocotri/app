...
- **Rich admin-controlled map data (2026-06)**: destinations now carry admin-editable `marker_icon`, bilingual `story` (local knowledge), `facts` (interesting facts, newline bullets) and `warnings` (safety notes) plus best_time/duration/difficulty/how_to_get and `activities`/`nearby_services` lists. Shared `MapLocationCard` surfaces story/facts/warnings in both the map preview (native popup + web selected card) and the destination detail page, bilingual via `pick()`. Admin form (`/admin/content` الوجهات) extended with icon-picker + list + bilingual multiline inputs. Backend generic admin CRUD persists all fields; idempotent startup backfill enriches existing seeded destinations. Verified: 65/65 backend tests + 6/6 frontend flows.
